...
def speak(text):
    ...
    is_speaking = True
    print(f"[ME]: {text}")
    # Send [ME]: {text} to Websocket
    asyncio.run(broadcast_to_clients({"type": "tts", "text": text, "role": "ME"}))
...
TRIGGER_KEYWORDS = ["hey you", "hey", "you"]
    ...
    with audio_stream:
        while True:
            ...
            if rec.AcceptWaveform(data):
                ...
                print(f"YOU: {text}")
                # Send YOU: {text} to Websocket
                asyncio.run(broadcast_to_clients({"type": "stt", "text": text, "role": "YOU"}))
...
if __name__ == "__main__":
    uvicorn.run("VoskAPI_demo:app", host="0.0.0.0", port=8086, reload=False)   